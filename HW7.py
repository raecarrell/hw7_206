...
# Your name: raegan carrell
# Your student id: 24124492
# Your email:rcarrell@umich.edu
...
def make_players_table(data, cur, conn):
    ...
    for player in data['squad']:
        ...
        """, (player_id, name, pos_id, birth_year, nat)
        )
    conn.commit()
...
def nationality_search(countries, cur, conn):
    place = ','.join(['?' for _ in countries])
    cur.execute(f"""
        SELECT name, position_id, nationality
        FROM Players
        WHERE nationality IN ({place})
    """, tuple(countries)
    )
    result = cur.fetchall()
    return result
